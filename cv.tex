\documentclass[8pt,a4paper]{moderncv}


\usepackage[scale=0.75]{geometry}
\usepackage[inline]{enumitem}
\usepackage{fontspec}
\setmainfont[
  Ligatures=TeX,
  Extension=.otf,
  BoldFont=cmunbx,
  ItalicFont=cmunti,
  BoldItalicFont=cmunbi,
]{cmunrm}
\usepackage[citestyle=authoryear,natbib]{biblatex}
\addbibresource{main.bib}

\usepackage[firstyear=2009,lastyear=2020]{moderntimeline}


\moderncvstyle{classic} 
\moderncvcolor{black}  

\firstname{Sergei}
\lastname{Kozlukov}
\title{Aspiring mathematician}
\address{5, Tsimlyanskaya}{109~559~Moscow}{Russia}
\mobile{+7~(900)~945~5929}
\email{newkozlukov@gmail.com}
\social[github]{newkozlukov}
\social[twitter]{newkozlukov}
\social[linkedin]{newkozlukov}
\extrainfo{https://newkozlukov.gitlab.io/}
%\photo[64pt]{pic.jpg}

\begin{document}
\maketitle

\section{Personal}
\cvline{Born}{Nov 14, 1996}
\cvline{Citizenship}{Russian}

\section{Industry experience}
\tlcventry{2018/4}{2018/8}{R\&D}{\citet{r3ds}}{Voronezh}{}{
    \textbf{Splines}, mesh deformation tools, facial emotions reconstruction;
    reproduced results of certain papers, built demos
    (technologies used: \textbf{\texttt{C++}}, \texttt{\citet{eigen}}, \texttt{Qt}, proprietary \texttt{3D}-stack);
    studied works of Carl de Boor's related to interpolation and approximation.
}

\section{Education}
\tlcventry{2009}{2012}{Data processing op.}{}{Voronezh}{}{\textbf{.NET development}, supervised by Dmitry
Babich}
\tlcventry{2014}{2018}{B.~Sc. in Applied Mathematics}{Voronezh State
  University}{Voronezh}{honours}{
  Mechanics department, then Applied Mathematics and
  Informatics program, then Nonlinear Dynamics department. 
  Functional analysis and perturbation theory,
  \textbf{supervised by} Dr. Prof. \textbf{Anatoly Baskakov}.
  Studied~\citep{baskakov1987theorem}, published:
  \begin{enumerate}
      \item
          Perturbations of the matrix-of-all-ones~\citep{Koz17}.
      \item Perturbations of Kronecker products of self-adjoint matrices with simple spectrum~\citep{Koz18}.
  \end{enumerate}
}
\tlcventry{2018}{2020}{M.~Sc. in Statistical Learning Theory}{Skoltech and Higher School of Economics}%
{Moscow}{}{
Investigating relationship of extendibility of geodesics
and distnace convexity, supervised by Thibaut Le Gouic.}

\section{Peer-reviewed publications}
\printbibliography[keyword=peerReviewed,heading=none]

\section{Soft skills}
\cvlistitem{\textbf{Math}.}
\cvlistitem{Only learning to communicate with people,
but I \emph{do} learn.}
\cvlistitem{Coming from academic environment, might need, at
    the early stages, some mentorship, as well as oportunity to
    ask stupid questions, and casual reminders that I shouldn't waste time
seeking a "perfect" solution.}

\section{Hard skills}
\cvlistitem{Math.}
\cvlistitem{Done some time with
    \texttt{Python},
    \texttt{C},
    \texttt{C++},
    \texttt{Java},
    \texttt{C\#+.NET},
    etc
}
\cvlistitem{
    Standard libraries, build systems, version control, tests, etc.
}
\cvlistitem{Algorithms and data structures
(a bit of ICPC, \citet{sedgewickPartI,segdewickPartII}, etc).}
\cvlistitem{Beginning to learn \texttt{pytorch} stack.}

\section{Research interests}
\cvlistitem{Linear analysis, perturbation theory, method of similar operators.}
\cvlistitem{B-Splines.}
\cvlistitem{Optimal control.}

\section{Subjects under study}
\cvlistitem{\textbf{Geodesics}, global curvature spaces,
    concentration inequalities, esp. variance inequality;
stability of barycenters.}
\cvlistitem{\textbf{Riemannian optimization}.}
\cvlistitem{\textbf{Geometric methods in deep learning}.}
\cvlistitem{\textbf{Hyperbolic deeplearning}.}
\cvlistitem{Projective dynamics, FEM, heat methods; anything related
to simulation of deformable objects;
anything related to \textbf{VFX}.}
\cvlistitem{Dual methods in optimization, accelerated methods.}
\cvlistitem{Optimal transportation theory (Monge-Kantorovich problems).}
\cvlistitem{Embedded Linux, U-Boot.}
\cvlistitem{Coq.}

\section{Other skills}
\cvlistitem{Networking in \(*\)-nix (\texttt{iproute2}, \texttt{aircrack-ng}, etc).}
\cvlistitem{Bits of \texttt{Kademlia}~\citep{maymounkov2002kademlia},
\texttt{cjdns}~\citep{delisle2012cjdns}, onion routing (at least,
concept-wise), \texttt{Tox}~\citep{tox}.}
\cvlistitem{Bits of systemd,
very scarce bits of linux kernel.}
\cvlistitem{Bits of security (LUKS, GPG, PKI, WoT, CoT, OTR, etc).}
\cvlistitem{Tiling WM+\texttt{neovim} workflow.}

\section{Languages}
\cvlanguage{Russian}{Native}{}
\cvlanguage{English}{Comfortable}{}

\section{Talks, conferences}
\nocite{*}
\printbibliography[keyword=talk,heading=none]

\newpage
\section*{External references}
\printbibliography[heading=none,notkeyword=my]
\end{document}
